  ...
  .post((req, res) => {
    knex("creation")
      .insert({
        ...
        image_url:
          "https://images.unsplash.com/flagged/photo-1561668038-2742fcef75d7?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2340&q=80",
      })
      .catch((error) => {
        console.log(error);
      });
...
router.delete("/:id", (req, res) => {
  knex("creation")
    ...
    .del()
    .then(() => {
      res.status(204).json({ message: "Creation successfully deleted." });
    })
    .catch(() => {
      res.status(500).json({ message: "Error deleting creation." });
      console.error(error);
    });
});
